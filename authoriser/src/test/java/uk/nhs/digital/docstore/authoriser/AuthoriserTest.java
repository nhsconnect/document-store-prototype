...
class AuthoriserTest {
    ...
    void shouldHandleRequestWhenTokenHasExceptingRoles() {
        var pcseAllowedResources =
                List.of("api-gateway-invocation-arn-1", "api-gateway-invocation-arn-2");
        var clinicalAllowedResources =
                List.of("api-gateway-invocation-arn-3", "api-gateway-invocation-arn-4");
        var authConfig = new AuthConfig(pcseAllowedResources, clinicalAllowedResources);
        ...
        statements.addAll(
                pcseAllowedResources.stream()
                        .map(IamPolicyResponse::allowStatement)
                        .collect(Collectors.toList()));
        statements.addAll(
                clinicalAllowedResources.stream()
        ...
        assertThat(response.getPolicyDocument())
                ...
                .isEqualTo(expectedResponse.getPolicyDocument());
    }
    ...
    void shouldDenyAllWhenTokenDoesNotHaveExpectedClaim() {
        ...
        var token = JWT.create().withSubject(principalId).withAudience("invalid audience");
