...
public class DocumentReferenceSearchE2eTest {
                ...
                Map.of(
                        ...
                        "Location",
                                new AttributeValue(
                                        String.format(
                                                "s3://%s/%s",
                                                aws.getDocumentStoreBucketName(), S3_KEY)),
                ...
                Map.of(
                        ...
                        "Location",
                                new AttributeValue(
                                        String.format(
                                                "s3://%s/%s",
                                                aws.getDocumentStoreBucketName(), "somewhere")),
    ...
    void returnsEmptyBundleWhenNoMatchesFound() throws IOException, InterruptedException {
        String expectedEmptySearchResponse =
                getContentFromResource("search/empty-nhs-number-response.json");
        var searchRequest =
                HttpRequest.newBuilder(
                                getBaseUri()
                                        .resolve(
                                                "DocumentReference?subject:identifier=https://fhir.nhs.uk/Id/nhs-number%7C9111231130"))
                        .GET()
    ...
    void returnsMatchingResults() throws IOException, InterruptedException {
        ...
        var searchRequest =
                HttpRequest.newBuilder(
                                getBaseUri()
                                        .resolve(
                                                "DocumentReference?subject:identifier=https://fhir.nhs.uk/Id/nhs-number%7C9000000009"))
                        .GET()
                        .header("Authorization", createBearerToken())
                        .build();
        ...
        assertThatJson(searchResponse.body())
                .whenIgnoringPaths(
                        "$.meta", "$.entry[*].resource.meta", "$.entry[*].resource.indexed")
                .isEqualTo(expectedSearchResponse);
        assertThatJson(searchResponse.body()).inPath("$.entry[0].resource.indexed").isString();
    }

    @Test
    void returnsBadRequestIfSearchParametersAreInvalid() throws IOException, InterruptedException {
        String expectedErrorResponse =
                getContentFromResource("errors/unrecognised-subject-identifier-system.json");
        var searchRequest =
                HttpRequest.newBuilder(
                                getBaseUri()
                                        .resolve(
                                                "DocumentReference?subject:identifier=unrecognised-subject-identifier-system%7C9000000009"))
                        .GET()
                        .header("Authorization", createBearerToken())
                        .build();
        ...
        assertThat(searchResponse.statusCode()).isEqualTo(400);
        assertThat(searchResponse.headers().firstValue("Content-Type"))
                .contains("application/fhir+json");
        assertThatJson(searchResponse.body()).isEqualTo(expectedErrorResponse);
    }

    private String createBearerToken() {
        String jwt = JWT.create().withClaim("email", "").sign(none());
        return "Bearer " + jwt;
    }
