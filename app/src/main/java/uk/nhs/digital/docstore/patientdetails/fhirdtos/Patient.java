...
public class Patient {
  ...
  public Optional<Name> getCurrentUsualName() {
    ...
    var nameOfTypeUsual =
        getNames().stream()
            .filter(name -> name.getUse().equalsIgnoreCase("Usual") && name.getPeriod().isCurrent())
            .findFirst();
  ...
  public Optional<Address> getCurrentHomeAddress() {
    ...
    Optional<Address> addressOfTypeHome =
        addresses.stream()
            .filter(
                (address) ->
                    Objects.equals(address.getUse(), "home") && address.getPeriod().isCurrent())
            .findFirst();
  ...
  public PatientDetails parse() throws IllFormedPatientDetailsException {
    ...
    return new PatientDetails(
        currentName
            .map(
                (name) ->
                    name.getGiven().stream().map(PatientName::new).collect(Collectors.toList()))
            .orElse(null),
        ...
        currentHomeAddress
            .map(
                (address) ->
                    address.getPostalCode() == null ? null : new Postcode(address.getPostalCode()))
            .orElse(null),
        new NhsNumber(id));
  }
}
