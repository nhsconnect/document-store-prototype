...
public class NHSNumberSearchParameterForm {
    private static final Pattern SUBJECT_IDENTIFIER_PATTERN =
            Pattern.compile("^(?<systempart>(?<system>.*?)(?<!\\\\)\\|)?(?<identifier>.*)$");
    ...
    public NHSNumberSearchParameterForm(Map<String, String> searchParameters) {
        String subject =
                Optional.ofNullable(searchParameters.get("subject:identifier"))
                        .or(() -> Optional.ofNullable(searchParameters.get("subject.identifier")))
                        .orElseThrow(
                                () -> new MissingSearchParametersException("subject:identifier"));
        this.matcher = SUBJECT_IDENTIFIER_PATTERN.matcher(subject);
    }

    public void validate() {
        if (!matcher.matches()
                || matcher.group("identifier").isBlank()
                || !(NHS_NUMBER_ID_PATTERN.matcher(matcher.group("identifier")).matches())) {
            throw new InvalidSubjectIdentifierException(matcher.group("systempart"));
        }
        if (matcher.group("systempart") != null
                && !NHS_NUMBER_SYSTEM_ID.equals(matcher.group("system"))) {
