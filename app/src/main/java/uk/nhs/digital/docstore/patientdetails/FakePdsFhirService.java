    ...
    public Patient fetchPatientDetails(String nhsNumber) throws JsonProcessingException {
        ...
        switch (nhsNumber) {
            case "9000000025":
                ...
                sensitiveIndex.publish(new SearchPatientDetailsAuditMessage(nhsNumber, 200, now));

                return new Patient(nhsNumber, "2010-10-22", null, List.of(restrictedPatientName));
            case "9111231130":
                sensitiveIndex.publish(new SearchPatientDetailsAuditMessage(nhsNumber, 500, now));
