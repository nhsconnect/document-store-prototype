...
describe("The upload summary component", () => {
    ...
    it("does not include failed uploads in the successful uploads list", async () => {
        const files = [makeTextFile("one", 100), makeTextFile("two", 101)]
        const documents = [makeDocument(files[0], documentUploadStates.SUCCEEDED), makeDocument(files[1], documentUploadStates.FAILED)]
...
const makeDocument = (file, uploadStatus) => {
    return {file, state: uploadStatus ?? documentUploadStates.SUCCEEDED, progress: 0, id:nanoid()}
}
