...
describe('test the findByNhsNumber method', () => {
    test('returns a list of documents associated with an NHS number', async () => {
        ...
        const getMock = jest.fn(() => {return responseBody})
        ...
        const resourceObject = {description: 'description', docStatus: 'final', type: typeObject, content: contentObject}
        const responseBody = {total: 1, entry: [{resource: resourceObject}]}
        const expectedReturnedDocumentList = [{description: resourceObject.description, type: snomedCode, url: retrieveUrl}]
        const returnedDocumentList = await apiClient.findByNhsNumber(nhsNumber)
        expect(getMock).toHaveBeenCalledWith(
            "doc-store-api",
            '/DocumentReference',
            expect.objectContaining(
                {headers: requestHeaders, queryStringParameters: queryStringParametersMock}))
