...
resource "aws_lambda_function" "document_manifest_lambda" {
  handler          = "uk.nhs.digital.docstore.handlers.testlambdaHandler::handleRequest"
  function_name    = "testlambdaHandler"
  runtime          = "java11"
  role             = aws_iam_role.lambda_execution_role.arn
  timeout          = 60
  memory_size      = 1000
  filename         = var.lambda_testlambdaHandler_jar_filename
#  source_code_hash = filebase64sha256(var.lambda_testlambdaHandler_jar_filename)
  layers           = [
